...
final class ChannelsAPIManager {
    static let shared = ChannelsAPIManager()
    private init() { }
    
    private let provider = MoyaProvider<ChannelsAPI>(session: Session(interceptor: AuthIntercepter.shared))
    
    
    func request<T: Decodable>(api: ChannelsAPI, responseType: T.Type) -> Single<Result<T?, ErrorResponse>> {
