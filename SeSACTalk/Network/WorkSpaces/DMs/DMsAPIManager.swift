...
final class DMsAPIManager {
    static let shared = DMsAPIManager()
    private init() { }
    
    private let provider = MoyaProvider<DMsAPI>(session: Session(interceptor: AuthIntercepter.shared))
    
    func request<T: Decodable>(api: DMsAPI, resonseType: T.Type) -> Single<Result<T?, ErrorResponse>> {
